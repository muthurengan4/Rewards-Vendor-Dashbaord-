...
async def get_partners(
    ...
    # Convert vendors to partner-like format (skip vendors that already have a partner entry)
    existing_vendor_ids = {p.get("vendor_id") for p in partners if p.get("vendor_id")}
    vendor_partners = []
    for v in vendors:
        v_data = serialize_doc(v)
        if v_data["id"] in existing_vendor_ids:
            continue  # Skip - already has a partner entry
...
async def startup_auto_seed():
    ...
    # --- Seed partners (DISABLED - custom vendor data is managed via admin/vendor panel) ---
    count = await db.partners.count_documents({})
    print(f"Database has {count} partner(s). Partner auto-seed is disabled (use admin panel).")
